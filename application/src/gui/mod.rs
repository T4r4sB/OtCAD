...
impl GuiSystem {
  ...
  pub fn get_child(control: &Rc<RefCell<dyn GuiControl>>, position: Position) -> Rc<RefCell<dyn GuiControl>> {
  ...
  pub fn on_draw(&mut self, draw_context: &mut DrawContext) {
    if let Some(root) = &self.root {
      ...
      root.on_message(GuiMessage::Draw(&mut draw_context.buffer, &self.color_theme));
    }
  }
  ...
  pub fn on_mouse_up(&mut self, position: Position) -> bool {
    if let Some(root) = &self.root {
      let maybe_pressed = self.get_pressed();
      let handler = if let Some(pressed) = &maybe_pressed {pressed.clone()} else {Self::get_child(&root, position)};
      if handler.borrow_mut().on_message(GuiMessage::MouseUp(position, Rc::downgrade(&self.job_system))) {
        self.updated = false;
  ...
  pub fn create_rc_by_control<Control: GuiControl>(control: Control) -> (Rc<RefCell<dyn GuiControl>>, Rc<RefCell<Control>>) {
