...
impl<'i, Pixel> ImageViewMut<'i, Pixel> {
  pub unsafe fn from_raw(memory: *mut Pixel, size: ImageSize, stride: usize) -> Self {
    Self {
      memory, size, stride, lifetime_marker: PhantomData,
    }
  }
  ...
  pub fn window_mut(&mut self, left_top: ImageSize, right_bottom: ImageSize) -> ImageViewMut<Pixel> {
    window!(self, left_top, right_bottom, ImageViewMut);
  }

  pub fn as_view(&self) -> ImageView<Pixel> {
    unsafe { return ImageView::from_raw(self.memory, self.size, self.stride); }
  }

  pub fn draw<SrcPixel>(
    &mut self, src: ImageView<SrcPixel>,
    position: Position,
    apply: impl Fn(&mut Pixel, &SrcPixel)
  ) {
    if position.0 >= self.size.0 as i32 || position.1 >= self.size.1 as i32
    || position.0 + src.size.0 as i32 <= 0 || position.1 + src.size.1 as i32 <= 0 {
        return;
    }
    ...
    self
      .window_mut((left_top_x.0, left_top_y.0), (right_bottom_x.0, right_bottom_y.0))
      .draw_same_size(
        src.window((left_top_x.1, left_top_y.1), (right_bottom_x.1, right_bottom_y.1)),
        apply
      );
  }
...
impl<'i, Pixel> LineIter<'i, Pixel> {
  unsafe fn from_raw(first: *const Pixel, last: *const Pixel, size_x: usize, stride: usize) -> Self {
    Self{first, last, size_x, stride, lifetime_marker: PhantomData}
  }
}
...
mod tests {
  ...
  fn test_draw_same_size() {
    ...
    dst.as_view_mut().draw(src.as_view(), (0, 0), |d,s| *d = *s);
  ...
  fn test_draw_same_size_shifted_neg_neg() {
    let (src, mut dst) = init_two_images();
    dst.as_view_mut().draw(src.as_view(), (-2, -2), |d,s| *d = *s);
    assert_eq!(dst.as_view()[0][0], 42);
