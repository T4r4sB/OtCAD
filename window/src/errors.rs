...
pub fn code_to_string(code: DWORD)-> String {
  unsafe {
    ...
    let size = FormatMessageW(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
                              NULL, code, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT) as u32, std::mem::transmute(&mut message_buffer), 0, 0 as *mut *mut i8);

    if size == 0 {
      LocalFree(message_buffer as HLOCAL);
      let new_code = GetLastError();
      return format!("Failed with code {}, can not get info because of code {}", code, new_code);
    }

    let maybe_str = std::ffi::OsString::from_wide(std::slice::from_raw_parts(message_buffer, size as usize)).into_string();
    LocalFree(message_buffer as HLOCAL);
    match maybe_str {
      Ok(str) => return str.to_string(),
      Err(error) => return format!("Failed recode error message with code {}, because of {:?}", code, error),
    }
  }
}
