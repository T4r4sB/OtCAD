...
fn draw_rhombe<T: Float + std::fmt::Display>(
  size: ImageSize, 
  top: &ProjectivePoint<T>, left: &ProjectivePoint<T>, right: &ProjectivePoint<T>, bottom: &ProjectivePoint<T>,
  left_top: &Curve<T>, left_bottom: &Curve<T>, right_top: &Curve<T>, right_bottom: &Curve<T>,
  ...
  let unbounded_top = if top.p.y <= T::zero() {  T::zero() } else { (top.p.y / top.a).ceil() };
  let unbounded_bottom = if bottom.p.y > size_y * bottom.a { size_y } else { T::min(size_y, (bottom.p.y / bottom.a).ceil()) };
  ...
  let y1 = {
    ...
    if r {
      minimal_neg_value(unbounded_top, unbounded_bottom, left_top.a, left_top.n.y, left_top.a * size_x * size_x + left_top.n.x * size_x + left_top.c)
    } else if l {
      minimal_neg_value(unbounded_top, unbounded_bottom, right_top.a, right_top.n.y, right_top.c)
    } else {
      if top.p.y <= T::zero() { T::zero() } else { real_y }
    }
  };
  ...
  let y2 = {
    ...
    if r {
      minimal_neg_value(unbounded_top, unbounded_bottom, -left_bottom.a, -left_bottom.n.y, -(left_bottom.a * size_x * size_x + left_bottom.n.x * size_x + left_bottom.c))
    } else if l {
      minimal_neg_value(unbounded_top, unbounded_bottom, -right_bottom.a, -right_bottom.n.y, -right_bottom.c)
    } else {
      if bottom.p.y > size_y * bottom.a { size_y } else { real_y }
    }
  };
  ...
  let unbounded_left = if left.p.x <= T::zero() {  T::zero() } else { (left.p.x / left.a).ceil() };
  let unbounded_right = if right.p.x > size_x * right.a { size_x } else { (right.p.x / right.a).ceil() };
  ...
  if uy1 < ul {
    ...
    let fx = minimal_neg_value(unbounded_left, unbounded_right, left_top.a, left_top.n.x, left_top.a * fy * fy + left_top.n.y * fy + left_top.c);
  ...
  if ul < uy2 {
    ...
    let fx = minimal_neg_value(unbounded_left, unbounded_right, left_bottom.a, left_bottom.n.x, left_bottom.a * fy * fy + left_bottom.n.y * fy + left_bottom.c);
    let mut x: usize = NumCast::from(fx).unwrap();
  ...
  if uy1 < ur {
    let fy = T::from(uy1).unwrap();
    let fx = minimal_neg_value(unbounded_left, unbounded_right, -right_top.a, -right_top.n.x, -(right_top.a * fy * fy + right_top.n.y * fy + right_top.c));
    let mut x: usize = NumCast::from(fx).unwrap();
  ...
  if ur < uy2 {
    ...
    let fx = minimal_neg_value(unbounded_left, unbounded_right, -right_bottom.a, -right_bottom.n.x, -(right_bottom.a * fy * fy + right_bottom.n.y * fy + right_bottom.c));
    let mut x: usize = NumCast::from(fx).unwrap();
...
fn draw_quadrant<T: Float + std::fmt::Display>(
  size: ImageSize, border_funcs: [&Curve<T>; 4], corners: [&ProjectivePoint<T>; 4], q: usize, 
  buffer: &mut[(usize, usize)]
) -> (usize, usize) {
  draw_rhombe(
    size, 
    corners[(5 - q) % 4], corners[(4 - q) % 4], corners[(6 - q) % 4], corners[(3 - q) % 4],
    border_funcs[(4 - q) % 4], border_funcs[(3 - q) % 4], border_funcs[(5 - q) % 4], border_funcs[(6 - q) % 4],
    buffer,
  )
}

fn use_quadrant_bounds(dst: &mut ImageViewMut<u32>, buffer: &[(usize, usize)], ybounds: (usize, usize), color: u32) {
...
fn use_quadrant_bounds_aa2(dst: &mut ImageViewMut<u32>, buffer: &[(usize, usize)], ybounds: (usize, usize), color: u32) {
...
fn use_quadrant_bounds_aa4(dst: &mut ImageViewMut<u32>, buffer: &[(usize, usize)], ybounds: (usize, usize), color: u32) {
...
pub fn draw_curve<T: Float + std::fmt::Display>(
  dst: &mut ImageViewMut<u32>, entity: &Entity<T>, color: u32, width: T,
  buffer: &mut[(usize, usize)], anti_aliasing: usize,
) {
  ...
  macro_rules! in_or_center {
    ($f: expr) => {
      if maybe_in_curve.is_some() {$f} else {center}
    };
  }
  ...
  let default_in_points = [
    in_or_center!(ProjectivePoint::new(Point::new(in_curve.n.x + in_d, in_curve.n.y), -in_curve.a - in_curve.a)),
    in_or_center!(ProjectivePoint::new(Point::new(in_curve.n.x, in_curve.n.y + in_d), -in_curve.a - in_curve.a)),
    in_or_center!(ProjectivePoint::new(Point::new(in_curve.n.x - in_d, in_curve.n.y), -in_curve.a - in_curve.a)),
    in_or_center!(ProjectivePoint::new(Point::new(in_curve.n.x, in_curve.n.y - in_d), -in_curve.a - in_curve.a)),
  ];

  let default_out_points = [
    ProjectivePoint::new(Point::new(-out_curve.n.x + out_d, -out_curve.n.y), out_curve.a + out_curve.a),
    ProjectivePoint::new(Point::new(-out_curve.n.x, -out_curve.n.y + out_d), out_curve.a + out_curve.a),
    ProjectivePoint::new(Point::new(-out_curve.n.x - out_d, -out_curve.n.y), out_curve.a + out_curve.a),
    ProjectivePoint::new(Point::new(-out_curve.n.x, -out_curve.n.y - out_d), out_curve.a + out_curve.a),
  ];
  ...
  let view_bounds = (dst.get_size().0 * anti_aliasing, dst.get_size().1 * anti_aliasing);
  match entity {
    Entity::Curve(_) => {
      for q in 0 .. 4 {
        let ys = draw_quadrant(
          view_bounds,
          [&in_curve, &default_border_curves[(q + 1) % 4] , &out_curve, &default_border_curves[q]],
          [&default_in_points[(q + 1) % 4], &default_in_points[q], &default_out_points[q], &default_out_points[(q + 1) % 4]], 
          q, buffer
        );
        use_quadrant!(ys);
      }
    ...
    Entity::CurveSegment(s) => {
      ...
      let begin_in_point = in_or_center!(ProjectivePoint::new(s.begin - begin_direction.normalize().scale(half_width), T::one()));
      let end_in_point = in_or_center!(ProjectivePoint::new(s.end - end_direction.normalize().scale(half_width), T::one()));
      let begin_out_point = ProjectivePoint::new(s.begin + begin_direction.normalize().scale(half_width), T::one());
      let end_out_point = ProjectivePoint::new(s.end + end_direction.normalize().scale(half_width), T::one());
      ...
      if begin_quadrant == end_quadrant && !s.big {
        ...
        let ys = draw_quadrant(
          ...
          [&end_in_point, &begin_in_point, &begin_out_point, &end_out_point], 
          q, buffer
        );
        use_quadrant!(ys);
      } else {
        ...
        let ys = draw_quadrant(
          view_bounds,
          [&in_curve, &begin_curve, &out_curve, &default_border_curves[q]],
          [&default_in_points[(q + 1) % 4], &begin_in_point, &begin_out_point, &default_out_points[(q + 1) % 4]], 
          q, buffer
        );
        use_quadrant!(ys);
        q = (q + 1) % 4;
        ...
        let ys = draw_quadrant(
          view_bounds,
          [&in_curve, &default_border_curves[(q + 1) % 4] , &out_curve, &end_curve],
          [&end_in_point, &default_in_points[q], &default_out_points[q], &end_out_point], 
          q, buffer
        );
        use_quadrant!(ys);
      }
...
mod tests {
  ...
  fn fuzzy_test_line() {
    ...
    for i in 0 .. 2_000 {
      ...
      draw_curve(&mut dst.as_view_mut(), &e, 0, rng.gen_range(0.001 .. 10.0), &mut buffer, 4);
    }
  }
    ...
    for i in 0 .. 10_000 {
      ...
      for y in 0 .. dst.get_size().1 {
        ...
        for x in 0 .. line.len() {
          ...
          let control = f32::max(f32::max(out_curve.get_value(p), in_curve.get_value(p)), f32::max(begin_curve.get_value(p), end_curve.get_value(p)));
          assert!((line[x] > 0 && control > -0.1) || (line[x] <= 0 && control < 0.1), "mismatch on seed {} curve {:?} and width {:?} at [{}:{}]", i, s, width, x, y);
        }
      }
    }
  }
    ...
    for i in 0 .. 10_000 {
      ...
      if angle2 > PI {
        for y in 0 .. dst.get_size().1 {
          ...
          for x in 0 .. line.len() {
            let p = Point::new(x as f32 ,y as f32);
            let control = f32::max(f32::max(out_curve.get_value(p), in_curve.get_value(p)), f32::min(begin_curve.get_value(p), end_curve.get_value(p)));
            assert!(
